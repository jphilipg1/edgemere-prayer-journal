...
function go(personId) { selected=personId; notice(''); render(true); }
...
function renderHome(v) {
 v.append(el('p','YOUR PERSONAL NOTEBOOK','meta'),el('h1','My Prayer Journal'),el('p','Meet. Listen. Pray. Remember. Follow up.','muted'));
 ...
 if(!planned.length)follow.append(el('p','No follow-ups planned. Set a reminder on a person’s page or after a meeting.','muted'));
 planned.forEach(p=>{const b=button('',()=>go(p.id),'person');b.append(el('strong',p.name),el('span',followLabel(p.followUp)));follow.append(b);});v.append(follow);
...
function prayerCard(p,r){
 const c=el('article',undefined,'card');c.append(el('span',r.answered?'Answered':'Praying','badge'),el('p',r.text),el('p',r.added?`Added ${dateLabel(r.added)}`:'Date added was not recorded in the earlier journal.','meta'));
 r.updates.forEach(u=>{const line=el('div',undefined,'update');line.append(el('span',dateLabel(u.date),'meta'),el('p',u.text));c.append(line);});
 ...
 const actions=el('div',undefined,'actions');actions.append(button('Edit prayer',()=>editPrayer(r)),button('Add update',()=>addUpdate(r.id)),button(r.answered?'Reopen prayer':'Mark answered',()=>{
 if(save(d=>{const x=d.people.find(x=>x.id===p.id).prayers.find(x=>x.id===r.id);x.answered=!x.answered;x.answeredOn=x.answered?today():null;(x.history||=[]).push({action:x.answered?'answered':'reopened',date:today()});})){render();notice(r.answered?'Prayer reopened.':'Prayer marked answered.');}}),button('Delete prayer',()=>confirmDelete('Delete this prayer?','This permanently deletes this prayer, its updates, and its answer history.',()=>save(d=>{const x=d.people.find(x=>x.id===p.id);x.prayers=x.prayers.filter(x=>x.id!==r.id);}), 'Delete prayer')));c.append(actions);return c;
}
function renderPerson(v,p){
 v.append(button('← All people',()=>go(null),'secondary back'),el('h1',p.name));const m=recent(p);v.append(el('p',m?`Last meeting · ${dateLabel(m.date)}${m.location?' · '+m.location:''}`:'No meetings recorded yet','muted'));
 const actions=el('div',undefined,'actions');actions.append(button('Edit person',()=>editPerson(p)),button('Delete person',()=>confirmDelete('Delete this person?','This permanently deletes this person and all their prayer needs, updates, meetings, and follow-up dates.',()=>save(d=>{d.people=d.people.filter(x=>x.id!==p.id);}), 'Delete person and records')));v.append(actions);
 const follow=el('section',undefined,'section');follow.append(heading('Next Follow-Up',button(p.followUp?'Change reminder':'Set reminder',()=>openFollowup(p.id))),el('p',p.followUp?followLabel(p.followUp):'No reminder set.','muted'));v.append(follow);
 for(const answered of [false,true]){const section=el('section',undefined,'section');section.append(heading(answered?'Answered Prayers':'Active Prayer Needs',answered?null:button('Add Prayer Need',()=>editPrayer(),'')));const prayers=p.prayers.filter(r=>r.answered===answered);if(!prayers.length)section.append(el('p',answered?'Answered prayers will stay here as part of your story.':'What can you pray about together?','muted'));prayers.forEach(r=>section.append(prayerCard(p,r)));v.append(section);}
 const meetings=el('section',undefined,'section');meetings.append(heading('Meeting History',button('Record Meeting',()=>addMeeting(),'')));if(!p.meetings.length)meetings.append(el('p','Record when you met and what you want to remember.','muted'));
 [...p.meetings].sort((a,b)=>b.date.localeCompare(a.date)).forEach(m=>{const c=el('article',undefined,'card');c.append(el('h3',dateLabel(m.date)));if(m.location)c.append(el('p',m.location,'meta'));if(m.notes)c.append(el('p',m.notes));meetings.append(c);});v.append(meetings);
}
function render(focus=false){const v=$('view');v.replaceChildren();const p=person();if(p)renderPerson(v,p);else{selected=null;renderHome(v);}if(locked)v.querySelectorAll('button,input').forEach(n=>n.disabled=true);if(focus){const h=v.querySelector('h1');h.tabIndex=-1;h.focus();window.scrollTo(0,0);}updateDueBanner();}
function form(title,fields,commit,afterSave){
 ...
 $('form').onsubmit=e=>{e.preventDefault();const values={};for(const f of fields){values[f.name]=$(f.name).value.trim();if(f.required&&!values[f.name]){$('form-error').textContent='Please fill in the required fields.';$(f.name).focus();return;}}if(commit(values)){$('editor').close();render(true);notice('Saved on this device.');afterSave?.();}else $('form-error').textContent='Not saved. Your text is still here. Resolve the storage issue shown on the page, then try again.';};$('editor').showModal();
}
...
function openFollowup(personId){
 ...
 const finish=value=>{if(save(d=>{d.people.find(x=>x.id===personId).followUp=value;})){dialog.close();render();notice(value?'Follow-up saved on this device.':'Reminder removed.');checkDue();}else error.textContent='Could not save the follow-up. Your meeting and earlier reminder remain unchanged.';};
