...
const CACHE = PREFIX + 'v10-scheduled-push-test';
...
self.addEventListener('fetch',event=>{
 ...
 if(service&&event.request.url.startsWith(service+'/')&&['/config','/sync','/test','/schedule-test','/delete'].includes(new URL(event.request.url).pathname)){event.respondWith(fetch(event.request));return;}
...
self.addEventListener('push',event=>{
 let type='followup';try{const value=event.data?.json()?.type;if(['test','scheduled-test'].includes(value))type=value;}catch{}
 // Never render server-supplied text, metadata or URLs.
 event.waitUntil(self.registration.showNotification('Prayer Journal',{body:type==='test'?'Notifications are working.':type==='scheduled-test'?'Scheduled notifications are working.':'You have follow-ups today.',tag:type==='test'?'epj-test':type==='scheduled-test'?'epj-scheduled-test':'epj-followup',icon:new URL('icon-192.png',BASE).href}));
});
